...
# Import all necessary models from the updated db.py
from db import SessionLocal, Token, Settings, Dashboard, AuditLog, create_audit_log
...
# -----------------------
# DATABASE SETTINGS HELPERS (NEW)
# -----------------------
def get_user_settings(user_id):
    """Retrieves all general settings for a user from the database."""
    session = SessionLocal()
    try:
...
def save_user_settings(user_id, settings_data):
    """Saves or updates the general settings for a user."""
    session = SessionLocal()
...
# -----------------------
# DASHBOARD SESSION HELPERS (NEW)
# -----------------------
def get_dashboard_sessions_db(user_id):
    """Retrieves the list of saved dashboard sessions/layouts for the user."""
    session = SessionLocal()
    try:
        # Order by last accessed descending to get the 'current' one first
        ...
        return [
            {
                ...
                "is_current": (sessions.index(dash) == 0) # The one with the latest last_accessed is considered current
            }
...
    save_token(user_id, token_data)
    ...
    # Log the successful connection/login event
...
async def get_settings_endpoint(user_id: str):
    """Endpoint to retrieve a user's saved settings."""
    settings = get_user_settings(user_id)
    return JSONResponse(settings)

@app.post("/api/settings")
async def save_settings_endpoint(request: Request, user_id: str):
    """Endpoint to save a user's general settings."""
    try:
        settings_data = await request.json()
        result = save_user_settings(user_id, settings_data)
        return JSONResponse(result, status_code=200)
    except Exception as e:
        return JSONResponse({"error": f"Failed to save settings: {e}"}, status_code=500)

@app.post("/api/cache/purge")
async def purge_cache_endpoint(user_id: str):
    """
    Endpoint to execute clearing the user's server-side cache/saved analyses.
    """
    session = SessionLocal()
    try:
        # NOTE: Implement actual server-side cache cleanup logic here
        # Example: Delete temporary records or files linked to user_id
        # session.query(TemporaryAnalysisCache).filter(TemporaryAnalysisCache.user_id == user_id).delete()
        # session.commit()
        
        print(f"Server-side analysis cache purge executed for user: {user_id}")
        return JSONResponse({"message": "Server-side cache purge simulated successfully."}, status_code=200)
    except Exception as e:
        session.rollback()
        return JSONResponse({"error": f"Failed to purge server cache: {e}"}, status_code=500)
    finally:
        session.close()

# -----------------------
# DASHBOARD SESSION ENDPOINTS (NEW)
# -----------------------
@app.get("/api/dashboard/sessions")
async def get_dashboard_sessions_endpoint(user_id: str):
    """
    Fetches the list of saved dashboard sessions for the current user.
    This is the first call after login to determine what to render.
    """
    if not user_id:
        return JSONResponse({"error": "User ID required for session retrieval"}, status_code=400)
    
    sessions = get_dashboard_sessions_db(user_id)
    
    if not sessions:
        return JSONResponse({
            "message": "No saved sessions found. Please create your first dashboard.",
            "sessions": [],
            "action": "PROMPT_NEW_DASHBOARD"
        })
...
# -----------------------
# SECURITY & PROFILE ENDPOINTS (NEW)
# -----------------------
@app.post("/api/security/change-password")
async def change_password_mock(request: Request, user_id: str):
    """Mocks a password change operation and logs the event."""
    # NOTE: Real implementation requires hashing and actual user model interaction.
    await request.json() 
    create_audit_log(user_id, "PASSWORD_CHANGE", ip_address=request.client.host)
    return JSONResponse({"message": "Password change simulation successful."}, status_code=200)

@app.post("/api/security/toggle-2fa")
async def toggle_2fa_mock(request: Request, user_id: str):
    """Mocks a 2FA toggle operation and logs the event."""
    body = await request.json()
    is_enabled = body.get("enabled", False)
    
    event_type = "2FA_ENABLED" if is_enabled else "2FA_DISABLED"
    create_audit_log(user_id, event_type, ip_address=request.client.host)
    
    return JSONResponse({"message": f"2FA status set to {is_enabled} (Simulated)."}, status_code=200)

@app.get("/api/security/logins")
async def get_recent_logins_endpoint(user_id: str):
    """Fetches recent login activity from the AuditLog table."""
    session = SessionLocal()
    try:
        # Fetch up to the last 10 audit log entries for the user
        logs = session.query(AuditLog).filter(AuditLog.user_id == user_id).order_by(AuditLog.timestamp.desc()).limit(10).all()
        ...
        formatted_logs = [
            {
                ...
                "location": f"IP: {log.ip_address}",
                "device": log.device_info or "Web Browser",
...
    print("Upload your .gguf model to the /models folder before deploying.")
    gpt_model = None
else:
...
        return JSONResponse({"error": "Local AI model not found on server"}, status_code=500)
...
        return JSONResponse({"error": str(e)}, status_code=500)# src/main.py
import os
import json
...
# DB imports (your existing db.py)
...
# JWT secret (set via env). In production, use a strong secret
...
def get_current_user(credentials: HTTPAuthorizationCredentials = Depends(security)):
    ...
    try:
        ...
        # return a lightweight object (avoid passing session-bound ORM objects around)
        return {"id": user.id, "email": user.email}
    finally:
        session.close()

# -----------------------
# DATABASE TOKEN HELPERS (unchanged)
# -----------------------
def save_token(user_id, token_data):
    session = SessionLocal()
    token_data["created_at"] = datetime.utcnow().isoformat()
...
# -----------------------
# DATABASE SETTINGS HELPERS (unchanged)
# -----------------------
def get_user_settings(user_id):
    session = SessionLocal()
    try:
...
# -----------------------
# DASHBOARD SESSION HELPERS (unchanged)
# -----------------------
def get_dashboard_sessions_db(user_id):
    session = SessionLocal()
    try:
...
# -----------------------
# GOOGLE TOKEN REFRESH (unchanged)
# -----------------------
async def get_valid_access_token(user_id):
    token_data = get_token(user_id)
    if not token_data:
...
# -----------------------
# GOOGLE OAUTH ROUTES (kept as-is, state=user_id supported)
# -----------------------
@app.get("/auth/google_sheets")
async def auth_google_sheets(user_id: str):
    url = (
...
async def connected_apps(user: dict = Depends(get_current_user)):
    # requires auth; returns google sheets connection status for current user
...
async def get_sheet_data(sheet_id: str, user: dict = Depends(get_current_user)):
    ...
    headers = {"Authorization": f"Bearer {access_token}"}
    # You might want to allow dynamic ranges; currently Sheet1
    url = f"https://sheets.googleapis.com/v4/spreadsheets/{sheet_id}/values:batchGet?ranges=Sheet1"
...
# -----------------------
# USER AUTH ROUTES (signup / login / me)
# -----------------------
...
async def purge_cache_endpoint(user: dict = Depends(get_current_user)):
    ...
    session = SessionLocal()
    try:
        # Implement actual server-side cache cleanup logic here if needed
        print(f"Server-side analysis cache purge executed for user: {user['id']}")
        return JSONResponse({"message": "Server-side cache purge simulated successfully."}, status_code=200)
    except Exception as e:
        session.rollback()
        return JSONResponse({"error": f"Failed to purge server cache: {e}"}, status_code=500)
...
# -----------------------
# DASHBOARD SESSION ENDPOINTS (auth)
# -----------------------
@app.get("/api/dashboard/sessions")
...
# -----------------------
# SECURITY & PROFILE ENDPOINTS (adjusted to auth)
# -----------------------
@app.post("/api/security/change-password")
...
# -----------------------
# AI ANALYSIS ENDPOINT (unchanged)
# -----------------------
@app.post("/ai/analyze")
async def ai_analyze(request: Request):
    if gpt_model is None:
